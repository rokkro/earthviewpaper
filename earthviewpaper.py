...
from ksetwallpaper import change_wallpaper_plasma
...
def linuxde(wallid):
    ...
    if de in ['ubuntu', 'gnome', 'cinnamon', 'pantheon', 'gnome-classic', 'budgie-desktop', 'default',
              'mate']:  # GTK3 based DE's. 'default' refers to Zorin.
        os.system("gsettings set org.gnome.desktop.background picture-uri file://" + dir + wallid + ".jpg")
    elif de.endswith('plasma'):
        change_wallpaper_plasma( dir + wallid + ".jpg")
    else:
        print(
...
def detectOS():
    if (platform.system() == 'Windows'):
        if (platform.architecture()[0] == '64bit'):
            ...
            ctypes.windll.user32.SystemParametersInfoW(20, 0, directory + wallid + ".jpg",
                                                       3)  # changes wallpaper on 64bit systems
        else:  # 32 bit
            directory = win(wallid)
...
def connectdl():  # attempts to retrieve wallids, splits into a list, randomly picks one, and downloads the image
    for i in range(0, 250000):
        try:
            # Retrieves list of wallids from pastebin, puts in temp.txt
            ...
            # Download randomly selected image id
